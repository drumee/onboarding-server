...
class Onboarding extends Entity {
  ...
  async save_user_info() {
    ...
    const lastName = this.input.get(Attr.lastname);
    const email = this.input.get(Attr.email);
    const countryCode = this.input.get(Attr.country);
  ...
  async get_countries() {
    const requestedLocale = this.input.get(Attr.locale) || this.session?.locale || 'en_US'; 
